<template>
  ...
  >
    ...
    <label for="assetsFieldHandle" class="block cursor-pointer text-sm text-center">
      <span class="hidden lg:block"> Arrastra tus archivos o da </span>
      <span class="underline">click aqui</span> para abrir el explorador de carpetas
    </label>
    <ul class="mt-4 lg:grid grid-cols-3 text-center" v-if="this.filelist.length" v-cloak>
      <li
        v-for="(file, index) in filelist"
        :key="index"
        class="text-xs text-indigo-500 p-1"
      >
        {{ file.name }}
        <button
          type="button"
